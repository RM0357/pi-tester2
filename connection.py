...
class SerialModem(object):
	...
	def __init__(self):
		...
		self.StatusPattern= re.compile( "#XPPP: (?P<Running>0|1),(?P<Connected>0|1)", re.MULTILINE )  # Todo: auch gegen DiagPatterns tauschen
		
		self.DiagPatterns= {
			'XPPP': re.compile( "#XPPP: (?P<Running>0|1),(?P<Connected>0|1)", re.MULTILINE ),
			'XSIM': re.compile( "%XSIM: (?P<state>[0-9]+)(,(?P<cause>[^,]+))?", re.MULTILINE ),
			...
			'CGDCONT': re.compile( "\+CGDCONT: (?P<cid>[0-9]+),(?P<PDP_type>[^,]+),(?P<APN>[^,]+),(?P<PDP_addr>[^,]+),(?P<d_comp>[^,]+),(?P<h_comp>[^,]+)?", re.MULTILINE )
		}
	...
	def ClearBuffer(self):
		...
		try:
			Response= self.Modem.read(1000).decode("utf-8").strip()    # Todo: länge definieren, kures + langes timeout? clear funktion mit kurzem timeout
		except:
